...
export const Assignees = () => {
	...
	const { members, reloadProjectTasks } = useProjectQueries(params.projectId as string)
	...
	const handleAssigneeToggle = (userId: string) => {
		setSelectedAssignees(prev => {
			...
			return isCurrentlySelected ? prev.filter(id => id !== userId) : [...prev, userId]
		})
	}

	const handlePopoverOpenChange = async (open: boolean) => {
		if (!open && JSON.stringify(selectedAssignees.sort()) !== JSON.stringify(task?.assignees?.map(a => a.id).sort())) {
	...
	const filteredMembers = allMembers?.filter(member => member.name?.toLowerCase().includes(filter.toLowerCase()))
