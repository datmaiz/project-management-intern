...
export const Board: React.FC<Props> = ({ projectId, projectName, statuses }) => {
	...
	const { activeTask, sensors, overColumnId, handleDragStart, handleDragEnd, handleDragOver } = useBoardDragAndDrop()
	...
	const handleColumnUpdate = (updatedColumn: IStatus) => {
		setColumns(prev => prev.map(status => (status.id === updatedColumn.id ? updatedColumn : status)))
	}
	...
	const handleTaskUpdate = async (taskId: string, updates: Partial<ITaskWithOptions>) => {
		try {
			if ('labels' in updates || 'size' in updates || 'priority' in updates) {
				await reloadProjectTasks()
			} else {
				setTasks(prev =>
					prev.map(task => (task.id === taskId ? { ...task, ...(updates as Partial<ITaskWithOptions>) } : task))
				)
			}
		} catch (error) {
		...
		<TaskDetailsProvider onTaskUpdate={handleTaskUpdate}>
			<div className='h-[calc(100vh-200px)]'>
				...
				<div className='flex gap-1 w-full overflow-x-auto py-1'>
					<div className={cn('flex gap-3', hiddenColumns.size > 0 ? 'h-[calc(100vh-175px)]' : 'h-[calc(100vh-155px)]')}>
						<DndContext
