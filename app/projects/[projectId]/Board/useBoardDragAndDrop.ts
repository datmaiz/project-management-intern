...
type SetTasksFunction = React.Dispatch<React.SetStateAction<ITaskWithOptions[]>>
...
export const useBoardDragAndDrop = () => {
	...
	const getPriorityTaskPositionOnNonEmptyColumn = (tasks: ITaskWithOptions[], active: any) => {
		...
		if (columnTasks.some(item => item.priority)) {
			const tasksWithHigherPriority = columnTasks.filter(
				task => (task.priority?.order ?? 0) > (active.data.current?.task.priority?.order ?? 0)
			)

			const tasksWithLowerPriority = columnTasks.filter(
				task => task.priority?.order === active.data.current?.task.priority?.order - 1
			)

			const tasksWithSamePriority = columnTasks.filter(
				task => task.priority?.order === active.data.current?.task.priority?.order
			)

			if (tasksWithSamePriority.length > 0) {
				const lastTaskWithSamePriority = tasksWithSamePriority[tasksWithSamePriority.length - 1]

				const index = columnTasks.findIndex(task => task.id === lastTaskWithSamePriority?.id)

				return moveTaskDown(index, lastTaskWithSamePriority?.id ?? '', overColumnId ?? '', tasks)
			} else if (tasksWithHigherPriority.length > 0) {
				const lastTaskWithHigherPriority = tasksWithHigherPriority[tasksWithHigherPriority.length - 1]
				const index = columnTasks.findIndex(task => task.id === lastTaskWithHigherPriority?.id)
				return moveTaskDown(index, lastTaskWithHigherPriority?.id ?? '', overColumnId ?? '', tasks)
			} else if (tasksWithLowerPriority.length > 0) {
				const targetTaskWithLowerPriority = tasksWithLowerPriority[0]
				const index = columnTasks.findIndex(task => task.id === targetTaskWithLowerPriority?.id)
				return moveTaskUp(index, targetTaskWithLowerPriority?.id ?? '', overColumnId ?? '', tasks)
			} else {
	...
	const getStatusMoveActivity = (userId: string, fromStatusId: string, toStatusId: string): TaskActivity => {
		return [
			{ type: 'user', id: userId },
			'moved this from',
	...
	const handleSameColumnDrag = async (context: DragTaskContext) => {
		const { tasks, setTasks, activeId, overId, overColumnId, active, over, overPosition, activePosition } = context
		...
		try {
			await tasksUtils.board.updatePosition(activeId as string, newStatusPosition as number)
			// create activity
			await createActivity({
				task_id: activeId as string,
				user_id: user?.id as string,
				content: getStatusMoveActivity(user?.id as string, activeColumnId as string, overColumnId as string),
			})
		} catch (error: any) {
	...
	const handleDifferentColumnDrag = async (context: DragTaskContext) => {
		...
		if (!activePriority && overPriority) {
			newStatusPosition = handleNonPriorityToPriorityMove(columnTasks, overColumnId, tasks)
		}
		// Case 2: Moving priority task to non-priority section
		else if (activePriority && !overPriority) {
			newStatusPosition = getPriorityTaskPositionOnNonEmptyColumn(tasks, active)
		}
		// Case 3: Moving between different priority levels
		else if (activePriority?.order > overPriority?.order) {
			newStatusPosition = handlePriorityLevelMove(columnTasks, active, overColumnId, tasks)
		}
		...
		try {
			await tasksUtils.board.moveTask(activeId as string, overColumnId as string, newStatusPosition as number)
	...
	const handleEmptyColumnDrag = async (context: DragTaskContext) => {
		...
		try {
			await tasksUtils.board.moveTask(activeId as string, overColumnId ?? '', 10000)
			// create activity
			await createActivity({
				task_id: activeId as string,
				user_id: user?.id as string,
	...
	const handleNonPriorityToPriorityMove = (
		...
		const firstNonPriorityTaskIndex = columnTasks.findIndex(task => task.id === firstNonPriorityTask?.id)

		if (firstNonPriorityTask) {
			return moveTaskUp(firstNonPriorityTaskIndex, firstNonPriorityTask?.id ?? '', overColumnId ?? '', tasks)
		} else {
			const lastTaskIndex = columnTasks.length - 1
			return moveTaskDown(lastTaskIndex, columnTasks[lastTaskIndex].id ?? '', overColumnId ?? '', tasks)
		}
	}
	...
	const handlePriorityLevelMove = (
		...
		const firstTaskWithLowerPriorityIndex = columnTasks.findIndex(task => task.id === firstTaskWithLowerPriority?.id)

		return moveTaskUp(firstTaskWithLowerPriorityIndex, firstTaskWithLowerPriority?.id ?? '', overColumnId ?? '', tasks)
	}
	...
	const handleNonPriorityColumnDrag = async (context: DragTaskContext) => {
		...
		const newStatusPosition = moveTaskDown(lastTaskIndex, lastTaskId ?? '', overColumnId ?? '', tasks)
		...
		try {
			await tasksUtils.board.moveTask(activeId as string, overColumnId as string, newStatusPosition ?? 0)
	...
	const handlePriorityColumnDrag = async (context: DragTaskContext) => {
		...
		const newStatusPosition = getPriorityTaskPositionOnNonEmptyColumn(tasks, active)
	...
	const handleDragEnd = async (event: DragEndEvent, tasks: ITaskWithOptions[], setTasks: SetTasksFunction) => {
		const { active, over } = event

		// Early return if invalid drag
		if (!over || active.data.current?.type !== 'task' || active.id === over.id) {
			setOverColumnId(null)
			return
		}
		...
		if (over.data.current?.type === 'task') {
			const isSameColumn = over.data.current?.task.status_id === active.data.current?.task.status_id
	...
	const handleDragOver = (event: DragOverEvent) => {
		...
		if (isOverTask) {
			...
			setOverColumnId(isSameColumn ? active.data.current?.task.status_id : over.data.current?.task.status_id)
