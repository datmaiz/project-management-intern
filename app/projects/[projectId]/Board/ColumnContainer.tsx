...
export const ColumnContainer = ({
	...
	const handleAddItem = async () => {
		...
		if (column.limit > 0 && columnTasks.length >= column.limit) {
			toast({
				variant: 'destructive',
				title: 'Limit Reached',
				description: `Cannot add more tasks to "${column.label}" column. Limit is ${column.limit}.`,
			})
			setInputValue('')
			setShowInput(false)
			return
	...
	return (
		...
		>
			<div className='p-2 space-y-1 flex-shrink-0'>
					...
					<div className='flex items-center gap-2'>
						...
						<TooltipProvider>
							<Tooltip>
								...
								{column.limit > 0 && columnTasks.length >= column.limit && (
									<TooltipContent>
										<p>Column limit {columnTasks.length > column.limit ? 'exceeded' : 'reached'}</p>
									</TooltipContent>
								)}
							</Tooltip>
						</TooltipProvider>
				...
				<div className='text-xs text-gray-500 dark:text-gray-400'>{column.description}</div>
			</div>
			...
			>
				<div
					className={cn(
						...
						isOver && 'bg-gray-200 dark:bg-gray-900 border border-dashed border-gray-400 dark:border-gray-600'
					)}
				>
