...
export const hasChanges = <T extends ComparableItem>(originalItems: T[], newItems: T[]): boolean => {
	const { itemsToAdd, itemsToUpdate, itemsToDelete } = compareAndUpdateItems(originalItems, newItems)
	return itemsToAdd.length > 0 || itemsToUpdate.length > 0 || itemsToDelete.length > 0
}
