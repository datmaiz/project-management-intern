...
export const projects = {
	...
	fields: {
		getStatuses: async (projectId: string) => {
			const supabase = await getSupabase()
			const { data, error } = await supabase.from('statuses').select('*').eq('project_id', projectId).order('order')
		...
		getPriorities: async (projectId: string) => {
			const supabase = await getSupabase()
			const { data, error } = await supabase.from('priorities').select('*').eq('project_id', projectId).order('order')
		...
		getSizes: async (projectId: string) => {
			const supabase = await getSupabase()
			const { data, error } = await supabase.from('sizes').select('*').eq('project_id', projectId).order('order')
	...
	getUserProjects: async (userId: string) => {
		...
		const allProjects = [...ownedProjects.data, ...memberProjects.data.map(row => row.project)]
